...
class Orchestrator:
    ...
    async def handle_query(self, session_id: str | None, user_text: str) -> Union[AskResponseData, AnswerResponseData]:
        ...
        decision = await self.decision_engine.evaluate(session, history_text)
        logger.info(f"Engine Decision: {decision} | Confidence: {session.confidence:.2f} | Gaps: {session.gaps_remaining}")
        ...
        if decision == "ASK":
            logger.info("Step 4: Generating next clinical question...")
            ...
            logger.info("Step 4: Generating final medical answer...")
